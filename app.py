...
# The API key is read from HF Spaces "Secrets" (set in Settings), never hardcoded here.
client = OpenAI(
...
def bridge_interface(native_lang, target_lang, sound_or_word):
    if not sound_or_word.strip():
        return "Please enter a sound, letter, or word you'd like to learn first."
    return get_pronunciation_bridge(native_lang, target_lang, sound_or_word)


demo = gr.Interface(
    fn=bridge_interface,
    inputs=[
        gr.Textbox(label="Your Native Language", placeholder="e.g. Indonesian, Arabic"),
        gr.Textbox(label="Language You're Learning", placeholder="e.g. English"),
        gr.Textbox(
            label="Sound / Letter / Word You Want to Learn",
            placeholder="e.g. the letter ث, or 'th' in the word think",
        ),
    ],
    outputs=gr.Textbox(label="Pronunciation Guide from LafazAI", lines=12),
    title="🌉 LafazAI — Bridging Pronunciation Across Languages",
    description=(
        "Struggling with an unfamiliar sound? LafazAI bridges the sound in "
        "the language you're learning with a sound you already know in your "
        "native language, using IPA notation and mouth-position guidance. "
        "Built on AMD Developer Cloud (ROCm 7.2, PyTorch 2.9) with inference "
        "via Fireworks AI (AMD-hosted)."
    ),
)
